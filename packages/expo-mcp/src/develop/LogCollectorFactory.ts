...
import { type LogCollector, type LogRecord } from './LogCollector.js';
...
export function createLogCollector(config: LogCollectorFactoryConfig): LogCollector {
  ...
  if (config.cdp) {
    collectors.push(new CdpLogCollector(config.cdp));
  }

  if (config.android) {
    collectors.push(new AndroidLogCollector(config.android));
  }

  if (config.iosSimulator) {
    collectors.push(new IosSimulatorLogCollector(config.iosSimulator));
  }
